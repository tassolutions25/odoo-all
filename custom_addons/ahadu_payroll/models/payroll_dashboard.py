...
class AhaduPayrollDashboard(models.TransientModel):
    ...
    def action_go_to_payslip_batches(self):
        return self.env.ref('ahadu_payroll.action_ahadu_payslip_run').sudo().read()[0]

    def action_go_to_cash_indemnity(self):
        return self.env.ref('ahadu_payroll.action_cash_indemnity_tracking').sudo().read()[0]

    def action_go_to_overtime(self):
        return self.env.ref('ahadu_payroll.action_ahadu_overtime_tracking').sudo().read()[0]

    def action_go_to_taxation_rules(self):
        return self.env.ref('ahadu_payroll.action_ahadu_payroll_tax_config').sudo().read()[0]
    ...
    def action_go_to_loan(self):
        return self.env.ref('ahadu_payroll.action_hr_loan').sudo().read()[0]
