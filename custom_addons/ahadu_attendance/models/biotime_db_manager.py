...
# the following code is working but not insert missout in report excel
# # -*- coding: utf-8 -*-
# import logging
...
#         """
#         Connects to MSSQL DB and gets First In / Last Out per day.
#         Includes Batch Commits to prevent timeouts.
#         """
#         _logger.info("🚀 Starting Direct DB Sync (Smart Mode with Batching)...")
...
#                     # 2. Logic B: Determine Check-Out
#                     check_out_final = False
                    
#                     if punch_count > 1 and last_punch_native != first_punch_native:
#                         check_out_local = local_tz.localize(last_punch_native)
#                         check_out_utc_aware = check_out_local.astimezone(pytz.UTC)
#                         check_out_final = check_out_utc_aware.replace(tzinfo=None)
                    
#                     elif punch_count == 1 and check_in_local.hour >= 12:
#                         check_out_final = check_in_final # Orphan Close

#                 except Exception as e:
#                     _logger.error(f"Time conversion error for {emp_code}: {e}")
#                     continue

#                 # --- 3. AUTO-CLOSE STUCK ATTENDANCE ---
#                 # Find previous days open attendance
#                 stuck_attendance = self.env['hr.attendance'].search([
#                     ('employee_id', '=', employee_odoo_id),
#                     ('check_out', '=', False),
#                     ('check_in', '<', check_in_final.date()) 
#                 ], limit=1)

#                 if stuck_attendance:
#                     auto_out_time = stuck_attendance.check_in + timedelta(hours=12)
#                     stuck_attendance.write({
#                         'check_out': auto_out_time,
#                         'check_out_method': 'System',
#                         'attendance_status': 'miss_out'
#                     })
#                     total_autoclosed += 1
...
#                 if not existing:
#                     # Loose check for same-day duplicates (UTC date matching)
#                     day_start = check_in_final.replace(hour=0, minute=0, second=0)
#                     day_end = check_in_final.replace(hour=23, minute=59, second=59)
...
#                                 'check_out_method': 'Biometric'
#                             }
#                             if check_in_final == check_out_final:
#                                 vals['attendance_status'] = 'miss_in'
#                             existing.write(vals)
#                             total_updated += 1
#                 else:
#                     vals = {
...
#                 # --- NEW: BATCH COMMIT EVERY 20 RECORDS ---
#                 # This saves progress so if timeout happens, we don't lose everything
...
#                   f"🆕 Created: {total_created}\n" \
#                   f"🔄 Updated: {total_updated}\n" \
#                   f"🔒 Auto-Closed: {total_autoclosed}"
...
#                 if len(sorted_missing) > 5:
#                     display_missing += f", ... (+{len(sorted_missing)-5} more)"
#                 msg += f"\n\n⚠️ IGNORED ({len(missing_employees)}) Unknown IDs: {display_missing}"
...
#             _logger.error(f"MSSQL Sync Error: {e}", exc_info=True)
#             if conn: conn.close()
#             # Even if it crashed, some records might be saved due to commit
#             return {"success": False, "message": f"Timeout/Error: {str(e)}. Records processed: {processed_count}"}
